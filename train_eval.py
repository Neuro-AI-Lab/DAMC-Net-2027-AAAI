...
from model import DualBranchSimAMNet
...
def train_model(model, train_loader, val_loader, class_names, device, num_epochs, save_dir):
    ...
    try:
        ...
        autocast_ctx = lambda: cuda_autocast(enabled=use_amp)
    ...
    try:
        ...
        scaler = CudaGradScaler(enabled=use_amp)
...
def run_one_subject_one_seed(subject_id, seed, num_epochs, batch_size,
    ...
    # 시퀀스는 자르지 않고 전체 구간(-0.5s ~ 2.6s, T=795)을 그대로 사용한다.
